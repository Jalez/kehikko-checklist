 ...
 *
 * There is a root element and there are two inert JSON islands. Every row, every
 * reason and every tick is built by React from what this program's own store
 * answers, because all of it depends on what is in that store — and because the
 * page has to be able to redraw after a tick without this file and the client
 * holding two versions of the same sentence.
 *
